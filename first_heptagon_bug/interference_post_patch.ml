...
module VarEnv = struct
  ...
  let remove_except_mem ix env =
    let x, fpred =
      match ix with
      | Ivar x -> x, is_mem_ivar
      | Ifield (Ivar x, _) -> x, (fun iy -> ivar_compare ix iy <> 0)
      | _ -> assert false
    in
    if mem x env then (
      let s = IvarSet.filter fpred (find x env) in
      if IvarSet.is_empty s then
        remove x env
      else
        add x s env
...
let compute_live_vars eqs =
  let add_alive (alive_vars, res) field_name eq read_ivars def_ivars =
    ...
    Option.iter (print_debug " (.%a)" Global_printer.print_qualname) field_name;
    print_debug "@,";
    print_debug_var_env "alive" alive_vars;
    let alive_vars_list = VarEnv.fold (fun _ ivs acc -> (IvarSet.elements ivs)@acc) alive_vars [] in
    let res = ((eq, field_name), alive_vars_list)::res in
    alive_vars, res
  in

  let field_aux eq def_ivar acc (field_name, ext) =
    let read_ivars = all_ivars_list (snd (InterfRead.read_extvalue () [] ext)) in
    add_alive acc (Some field_name) eq read_ivars [Ifield (def_ivar, field_name)]
  in
  ...
  let aux acc eq =
    ...
    | Estruct field_val_list ->
        let def_ivar =
          match InterfRead.def eq with [x] -> Ivar x | _ -> assert false
        in
        let alive_vars, res =
          List.fold_left (field_aux eq def_ivar) acc (List.rev field_val_list)
        in
        let alive_vars = VarEnv.remove_except_mem def_ivar alive_vars in
        alive_vars, res

    | _ ->
      let read_ivars = all_ivars_list (InterfRead.read eq) in
      let def_ivars = InterfRead.def_ivars eq in
      add_alive acc None eq read_ivars def_ivars
  in
  let add_mem x env =
